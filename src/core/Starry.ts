...
import { StarryProvider, GlobalFileNames } from "./webview/StarryProvider"
import { ApiConfiguration } from "../shared/api"
import { AutoApprovalSettings } from "../shared/AutoApprovalSettings"
import { HistoryItem } from "../shared/HistoryItem"
import { StarryAskResponse } from "../shared/WebviewMessage"
...
import {
	...
	StarryAsk,
	StarryApiReqInfo,
	StarryApiReqCancelReason,
	StarryAskUseMcpServer,
	StarrySayBrowserAction,
	StarrySayTool,
    BrowserAction,
	BrowserActionResult,
    browserActions
...
// Define the working directory and use the desktop if there is no workspace
const cwd =
	vscode.workspace.workspaceFolders?.map((folder) => folder.uri.fsPath).at(0) ?? path.join(os.homedir(), "Desktop") // may or may not exist but fs checking existence would immediately ask for permission which would be bad UX, need to come up with a better solution
// Define the type of tool response and user content
...
export class Starry {
	readonly taskId: string // unique identifier for the task
	api: ApiHandler
	private didEditFile: boolean = false
	customInstructions?: string // custom instructions for the task
	private providerRef: WeakRef<StarryProvider>
	private terminalManager: TerminalManager
	private urlContentFetcher: UrlContentFetcher
	private browserSession: BrowserSession
	private diffViewProvider: DiffViewProvider //
	apiConversationHistory: Anthropic.MessageParam[] = [] //
	starryMessages: StarryMessage[] = [] // for webview
	...
	private askResponse?: StarryAskResponse
	private askResponseText?: string
	private askResponseImages?: string[]
	private consecutiveAutoApprovedRequestsCount: number = 0

	private currentStreamingContentIndex = 0 // 当前流式内容索引
	private assistantMessageContent: AssistantMessageContent[] = [] // 助手消息内容
	private presentAssistantMessageLocked = false // 是否锁定当前助手消息
	private presentAssistantMessageHasPendingUpdates = false // 当前助手消息是否有待处理的更新
	...
	private didCompleteReadingStream = false // 是否完成了流式读取

	constructor(
		provider: StarryProvider,
	...
	private async startTask(task?: string, images?: string[]) {
		// conversationHistory (for API) and starryMessages (for webview) need to be in sync
		// if the extension process were killed, then on restart the starryMessages might not be empty, so we need to set it to [] when we create a new Starry client (otherwise webview would show stale messages from previous session)
	...
	async say(type: StarrySay, text?: string, images?: string[], partial?: boolean): Promise<undefined> {
		if (this.abort) {
			throw new Error("Starry instance aborted")
		}
		if (partial !== undefined) {
			const lastMessage = this.starryMessages.at(-1)
			...
			if (partial) {
				if (isUpdatingPreviousPartial) {
					...
					await this.addToStarryMessages({ ts: sayTs, type: "say", say: type, text, images, partial })
					await this.providerRef.deref()?.postStateToWebview()
				}
			} else {
				...
				if (isUpdatingPreviousPartial) {
					...
					const sayTs = Date.now()
					this.lastMessageTs = sayTs
					await this.addToStarryMessages({ ts: sayTs, type: "say", say: type, text, images })
					await this.providerRef.deref()?.postStateToWebview()
				}
			}
		} else {
	...
	async ask(
		...
		text?: string,
		partial?: boolean,
	): Promise<{ response: StarryAskResponse; text?: string; images?: string[] }> {
		// If this Starry instance was aborted by the provider, then the only thing keeping us alive is a promise still running in the background, in which case we don't want to send its result to the webview as it is attached to a new instance of Starry now. So we can safely ignore the result of any active promises, and this class will be deallocated. (Although we set Starry = undefined in provider, that simply removes the reference to this instance, but the instance is still alive until this promise resolves or rejects.)
		if (this.abort) {
			throw new Error("Starry instance aborted")
		}
		let askTs: number
		if (partial !== undefined) {
			...
			if (partial) {
				if (isUpdatingPreviousPartial) {
					...
					await this.addToStarryMessages({ ts: askTs, type: "ask", ask: type, text, partial })
					await this.providerRef.deref()?.postStateToWebview()
				...
				if (isUpdatingPreviousPartial) {
					...
					*/
					...
					askTs = Date.now()
					this.lastMessageTs = askTs
					await this.addToStarryMessages({ ts: askTs, type: "ask", ask: type, text })
					await this.providerRef.deref()?.postStateToWebview()
				}
			}
		} else {
	...
	private async addToStarryMessages(message: StarryMessage) {
		this.starryMessages.push(message)
		await this.saveStarryMessages()
	}

	private async overwriteClineMessages(newMessages: StarryMessage[]) {
		this.starryMessages = newMessages
		await this.saveStarryMessages()
	}

	private async saveStarryMessages() {
		try {
			const filePath = path.join(await this.ensureTaskDirectoryExists(), GlobalFileNames.uiMessages)
			await fs.writeFile(filePath, JSON.stringify(this.starryMessages))
			// combined as they are in ChatView
			const apiMetrics = getApiMetrics(combineApiRequests(combineCommandSequences(this.starryMessages.slice(1))))
			const taskMessage = this.starryMessages[0] // first message is always the task say
			...
			console.error("Failed to save starry messages:", error)
		}
	}
	...
	private async initiateTaskLoop(userContent: UserContent): Promise<void> {
		...
		while (!this.abort) {
			...
			// There is a MAX_REQUESTS_PER_TASK limit to prevent infinite requests, but Starry is prompted to finish the task as efficiently as he can.
			...
			if (didEndLoop) {
				...
				// this.say(
				// 	"tool",
				// 	"Starry responded with only text blocks but has not called attempt_completion yet. Forcing him to continue with task..."
				// )
				nextUserContent = [
					{
						type: "text",
	...
	async recursivelyMakeClineRequests(
		...
		if (this.consecutiveMistakeCount >= 3) {
			if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					subtitle: "Error",
					message: "Starry is having trouble. Would you like to continue the task?",
				})
			}
			const { response, text, images } = await this.ask(
				...
				this.api.getModel().id.includes("claude")
					...
					: "Starry uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 3.5 Sonnet for its advanced agentic coding capabilities.",
			)
		...
		if (
			...
			if (this.autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					...
					message: `Starry has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests.`,
				})
			}
			await this.ask(
				"auto_approval_max_req_reached",
				`Starry has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests. Would you like to reset the count and proceed with the task?`,
			)
		...
		const previousApiReqIndex = findLastIndex(this.starryMessages, (m) => m.say === "api_req_started")
		...
		const lastApiReqIndex = findLastIndex(this.starryMessages, (m) => m.say === "api_req_started")
		this.starryMessages[lastApiReqIndex].text = JSON.stringify({
			request: userContent.map((block) => formatContentBlockToMarkdown(block)).join("\n\n"),
		} satisfies StarryApiReqInfo)
		await this.saveStarryMessages()
		await this.providerRef.deref()?.postStateToWebview()
		...
		try {
			...
			const updateApiReqMsg = (cancelReason?: StarryApiReqCancelReason, streamingFailedMessage?: string) => {
				this.starryMessages[lastApiReqIndex].text = JSON.stringify({
					...JSON.parse(this.starryMessages[lastApiReqIndex].text || "{}"),
			...
			const abortStream = async (cancelReason: StarryApiReqCancelReason, streamingFailedMessage?: string) => {
    ...
    async handleWebviewAskResponse(askResponse: StarryAskResponse, text?: string, images?: string[]) {
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		if (previousApiReqIndex >= 0) {
			const previousRequest = this.starryMessages[previousApiReqIndex]
			if (previousRequest && previousRequest.text) {
				const { tokensIn, tokensOut, cacheWrites, cacheReads }: StarryApiReqInfo = JSON.parse(
    ...
    async presentAssistantMessage() {
		...
		switch (block.type) {
			...
			case "tool_use":
				...
				const askApproval = async (type: StarryAsk, partialMessage?: string) => {
				...
				switch (block.name) {
					...
					case "replace_in_file": {
						...
						try {
							...
							const sharedMessageProps: StarrySayTool = {
							...
							if (block.partial) {
								if (action === "launch") {
									...
									await this.say(
										...
										} satisfies StarrySayBrowserAction),
										undefined,
										block.partial,
									)
								}
					...
					case "use_mcp_tool": {
						...
						try {
							if (block.partial) {
								...
								} satisfies StarryAskUseMcpServer)
						...
						try {
							...
							if (block.partial) {
								if (command) {
									...
									// const secondLastMessage = this.starryMessages.at(-2)
    ...
    async sayAndCreateMissingParamError(toolName: ToolUseName, paramName: string, relPath?: string) {
        ...
        async removeLastPartialMessageIfExistsWithType(type: "ask" | "say", askOrSay: StarryAsk | StarrySay) {
            const lastMessage = this.starryMessages.at(-1)
            if (
                lastMessage?.partial &&
